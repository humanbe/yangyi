 ...
 * @since 3.0.2
 */
public class JedaIpAddressMatcher implements RequestMatcher {
	
	/** 日志输出 */
	final Logger logger = Logger.getLogger(JedaIpAddressMatcher.class);
     ...
     */
    public JedaIpAddressMatcher(String ipAddress) {
    ...
    public boolean matches(HttpServletRequest request) {
    	...
    	if(null != sClientIp){
    		remoteAddress = parseAddress(sClientIp);
    	}else{
    		//非F5配置的获取方法
    		remoteAddress = parseAddress(request.getRemoteAddr());
    	}
		// debug返回值.
		if (logger.isEnableFor("Common0002")) {
			logger.log("Common0002", "客户端IP=" + sClientIp);
		}
