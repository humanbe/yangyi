...
public class WebQueueService {
	...
	/**
	 * 查询某段时间内各个集群服务每天队列的峰值列表	 * 其中集群服务指多台机器的队列数据或者多个	 * weblogic服务的队列数据	 * @param aplCode 系统编号
	 * @param startDate 开始时间	 * @param endDate 结束时间
	 * @param clusterServers 集群服务器列表	 */
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, value = "maoprpt")
	public List<Map<String, String>> queryWebQueuePeak(String aplCode, String startDate,
		...
		sql.append("	select j.apl_code as \"aplCode\", ")
			...
			.append("	       max(j.used_queue) as \"usedValue\" ")
			...
			.append("	               max(to_number(t.used_queue)) as used_queue, ")
			.append("	               max(t.weblogic_port) as weblogic_port ")
			.append("	          from web_queue t ")
	...
	/**
	 * 查询指定服务器的指定服务的当日五分钟间隔weblogic队列使用数数据	 * @param aplCode 系统编号
	 * @param endDate 日报时间
	...
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, value = "maoprpt")
	public List<Map<String, String>> queryWebQueue4One(String aplCode, String endDate, String srvCode, Boolean existPortFlag, List<String> servers){
		...
		sql.append("	select T.monitor_time as \"monitorTime\", ")
			...
			.append("               max(to_number(trim(T.used_queue))) as \"usedQueue\" ")
			.append("	  from (select ")
			...
			.append("	               max(to_number(trim(used_queue))) as used_queue ")
			.append("                 from web_queue t ")
			.append("                 where apl_code= :aplCode ")
		...
		sql.append("                ) T ");
		sql.append("   group by T.monitor_time, T.weblogic_server");
		if(existPortFlag){
	...
	/**
	 * 查询当日集群服务器的指定服务的五分钟间隔weblogic队列使用数数据	 * 集群服务器，通过集群服务名获取多个服务，获取时点的最大值	 * @param aplCode 系统编号
	 * @param endDate 结束时间
	 * @param clusterServers 集群服务器列表	 */
	@SuppressWarnings("unchecked")
	@Transactional(readOnly = true, value = "maoprpt")
	public List<Map<String, String>> queryWebQueue4More(String aplCode, String endDate, List<String> clusterServers){
		StringBuilder sql = new StringBuilder();
		sql.append("	select T.monitor_time as \"monitorTime\", ")
			.append("               w.cluster_server as \"weblogicServer\", ")
			.append("               max(to_number(trim(T.used_queue))) as \"usedQueue\" ")
			.append("	  from (select ")
			.append("                 max(apl_code) as apl_code, ")
			.append("	               to_char(to_date(trunc(to_char(to_date(substr(monitor_time,0,5),'HH24:mi'),'yyyyMMddHH24mi')/ 5) *5 + 1, 'yyyymmddhh24mi'),'hh24:mi') as monitor_time, ")
			.append("	               max(srv_code) as srv_code, ")
			.append("                 max(weblogic_server) as weblogic_server, ")
			.append("	               max(to_number(trim(used_queue))) as used_queue ")
			.append("                 from web_queue ")
			.append("                 where apl_code= :aplCode ")
			.append("	               and monitor_date = :endDate ")
			.append("	               and to_date(to_char(to_date(trunc(to_char(to_date(substr(monitor_time,0,5),'HH24:mi'),'yyyyMMddHH24mi') / 5) *5 + 1, 'yyyymmddhh24mi'),'hh24:mi'),'hh24:mi') > to_date('00:00','hh24:mi') ")
			.append("	               and to_date(to_char(to_date(trunc(to_char(to_date(substr(monitor_time,0,5),'HH24:mi'),'yyyyMMddHH24mi') / 5) *5 + 1, 'yyyymmddhh24mi'),'hh24:mi'),'hh24:mi') < to_date('23:59','hh24:mi') ")              
			.append("	               group by monitor_time, srv_code, weblogic_server ) T, ")
			.append("       weblogic_conf w ")
