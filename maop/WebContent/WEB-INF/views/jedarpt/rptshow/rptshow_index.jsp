...
//页面初始化var init = function(){
	...
	//加载动态查询表单	rptShowIndexCreateForm();
	//加载动态列表	rptShowIndexCreateGridColAndDsfields();
	//重新配置store动态fields
	...
	//加载数据源	Ext.getCmp('rptShowIndexGrid'+report_id).getStore().load(); 
...
JeadReportList = Ext.extend(Ext.Panel, {
	...
	constructor : function(cfg) {// 构造方法		Ext.apply(this, cfg);
		...
		// 设置基类属性		JeadReportList.superclass.constructor.call(this, {
			layout : 'border',
			border : false,
			autoScroll : true,
...
//获取动态列名及数据源字段列表function rptShowIndexCreateGridColAndDsfields(){
	Ext.Ajax.request({
		method : 'POST',
...
function rptShowIndexReconfigureGird(sheetName){
	...
	//重新绑定bbar的store到新实例化的数据源	Ext.getCmp('rptShowIndexGrid'+reportId).getBottomToolbar().bind(rptShowIndexGridStore);
}
//获取动态查询表单function rptShowIndexCreateForm(){
	Ext.Ajax.request({
		scope : this,
		method : 'POST',
