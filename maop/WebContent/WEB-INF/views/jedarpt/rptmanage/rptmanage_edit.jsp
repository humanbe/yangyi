...
var panelOneEdit = null ;
var panelTwoEdit = null ;
var panelThreeEdit = null ;
var panelFourEdit = null ;

var parmEditGridEdit = null ; //报表参数
var columnEditGridEdit = null ; //报表字段
var reportRuleGridEdit = null ; //报表规则
var reportRoleGridEdit = null ; //报表角色
...
// 定义新建表单
JedaRptEditForm = Ext.extend(Ext.FormPanel, {
	...
	constructor : function(cfg) {// 构造方法
		...
		// 参数配置列表
		parmEditGridEdit = new Ext.grid.EditorGridPanel({
			id : 'param_edit_grid_edit',
			region : 'center',
			border : true,
			height : 225,
			loadMask : true,
			...
			listeners : {
			    ...
			    cellclick : function(grid, rowIndex, columnIndex, e) {
			    	var oldValue = grid.getStore().getAt(rowIndex).get('param_dic_item_edit');
					var params = {
						rowIndex : rowIndex ,
						oldValue : oldValue
					};
					switch (columnIndex) {
					case 4:
						app.loadWindow('${ctx}/${frameworkPath}/item/rptParamForEdit', params);
						break;
					default:
						break;
					}
		...
		// 显示字段配置列表
		columnEditGridEdit = new Ext.grid.EditorGridPanel({
			id : 'column_edit_grid_edit',
			region : 'center',
			border : true,
			height : 600,
			loadMask : true,
			...
			listeners : {
			    ...
			    //编辑后完成后处理事件
				'afteredit' : function(e) {
					var len = columnEditGridEdit.getStore().data.items.length;
					//字段排序验证
					if(e.field == 'column_sort'){
						...
						if(sort==null || sort==''){
							...
							var num1 = 0;
							for(var i=0;i<len;i++){
								if(columnEditGridEdit.getStore().data.items[i].data.column_sort==sort)
									num1++;
							}
							if(num1>1){
								Ext.Msg.show( {
					...
					if(e.field == 'column_ch_name'){
						...
						if(chName==null || chName==''){
							...
							var num2 = 0;
							for(var m=0;m<len;m++){
								if(columnEditGridEdit.getStore().data.items[m].data.column_ch_name==chName)
									num2++;
							}
							if(num2>1){
								Ext.Msg.show( {
			    ...
			    cellclick : function(grid, rowIndex, columnIndex, e) {
			    	var oldValue = grid.getStore().getAt(rowIndex).get('column_dic_item_edit');
					var params = {
						rowIndex : rowIndex ,
						oldValue : oldValue
					};
					switch (columnIndex) {
					case 6:
						app.loadWindow('${ctx}/${frameworkPath}/item/rptColumnForEdit', params);
						break;
					default:
						break;
					}
		...
		// 报表规则列表
		reportRuleGridEdit = new Ext.grid.EditorGridPanel({
			region : 'center',
			height : 700,
			border : false,
			loadMask : true,
		...
		// 实例化数据列表组件
		reportRoleGridEdit = new Ext.grid.GridPanel({
			region : 'center',
			height : 700,
			border : false,
			loadMask : true,
            ...
            items:[{
                ...
                {	columnWidth:.96,
					...
					listeners : {
						scope : this,
						'change' : function() {
							var sql = Ext.getCmp("report_sql_edit").getValue();
							var params = [];
							var records = parmEditGridEdit.getStore();
							...
							this.paramEditGridStore.baseParams.reportId = '';
							this.paramEditGridStore.reload();
							this.columnEditGridStore.baseParams.sql = sql;
							this.columnEditGridStore.baseParams.reportId = '';
					...
					fieldLabel : '报表已关联规则',
					hidden : true,
					id : 'jeda_rpt_rules_edit'
				},parmEditGridEdit]
            }]
	    });   
		
		// 第二步
	 	panelTwoEdit = new Ext.Panel({  
	 		title:'导出模板信息',
            layout:'column',
            border:false,
					...
					listeners : {
						...
						'fileselected' : function(fileField, path) {
							...
							if(fileType=='xls' || fileType=='XLS'){
								Ext.getCmp('template_path_edit').setValue(path);
								var templateName = path.substring(path.lastIndexOf('\\') + 1);
								Ext.getCmp('template_en_name_edit').setValue(templateName);
								Ext.getCmp('start_row_num_edit').setValue('1');
								Ext.getCmp('start_col_num_edit').setValue('1');
							}else{
								Ext.getCmp('template_path_load_edit').setValue('');
								Ext.Msg.show({
									title : '<fmt:message key="message.title" />',
	 	...
	 	// 第三步 - 定制报表规则
	 	panelThreeEdit = new Ext.Panel({  
	 		title : '规则制定',
            layout:'column',
            border:false,
            defaults:{bodyStyle:'padding-left:0px;padding-top:5px'},
			items : reportRuleGridEdit,
			listeners : {
				scope : this,
				'activate' : function(panel) {
					var records = reportRuleGridEdit.getStore().query('checked', true).getRange();
			        setTimeout(function(){
			        	reportRuleGridEdit.getSelectionModel().selectRecords(records, false);
			        }, 500);
				}
			}
	    });  
	    ...
	    // 第四步 - 角色授权
	 	panelFourEdit = new Ext.Panel({  
	 		title : '角色授权',
            layout:'column',
            border:false,
            defaults:{bodyStyle:'padding-left:0px;padding-top:5px'},
			items : [reportRoleGridEdit,{
				xtype : 'textfield',
				hidden : true, 
				id : 'report_params_edit'
			},{
				xtype : 'textfield',
				hidden : true, 
				id : 'report_columns_edit'
			},{
				xtype : 'textfield',
				hidden : true, 
				id : 'report_rules_edit'
			},{
				xtype : 'textfield',
				hidden : true, 
				id : 'report_roles_edit'
			},{
				xtype : 'textfield',
				hidden : true, 
				name : 'template_path',
				id : 'template_path_edit'
			},{
				xtype : 'textfield',
				hidden : true, 
				name : 'report_code'
			...
			listeners : {
				scope : this,
				'activate' : function(panel) {
					var records = reportRoleGridEdit.getStore().query('checked', true).getRange();
			        setTimeout(function(){
			        	reportRoleGridEdit.getSelectionModel().selectRecords(records, false);
			        }, 500);
				}
			}
	    });  
	    ...
	    function cardHandler(direction) {  
	        ...
	        var btnPrev = Ext.getCmp("jedaRpt_movePrev_edit");  
	        var btnSave = Ext.getCmp("jedaRpt_moveSave_edit");  
	        var btnNext = Ext.getCmp("jedaRpt_moveNext_edit");  
	        if (i == 0) {  
	            btnSave.hide();  
	            btnNext.enable();  
	            btnPrev.disable(); 
	        ...
	        if(i == 1){
	            ...
	            btnPrev.enable(); 
	            // 报表名称
	            var reportName = Ext.getCmp('report_name_edit').getValue();
				if (reportName.trim() == '') {
					Ext.Msg.show({
						title : '<fmt:message key="message.title" />',
						msg : '报表名称不能为空！',
				...
				// 展现路径
				var reportPath = Ext.getCmp('parent_menu_name_edit').getValue();
				if (reportPath.trim() == '') {
					Ext.Msg.show({
						title : '<fmt:message key="message.title" />',
						msg : '展现路径不能为空！',
				...
				// 查询SQL
				var reportSql = Ext.getCmp('report_sql_edit').getValue();
				if (reportSql.trim() == '') {
					Ext.Msg.show({
						title : '<fmt:message key="message.title" />',
						msg : '查询SQL不能为空！',
    			...
    			Ext.getCmp("report_params_edit").setValue( Ext.util.JSON.encode(params));
	        }
	        if(i == 2){
    			btnSave.hide();  
	            btnNext.enable();   
				...
				//报表字段
	    		var columns = [];
				var records = columnEditGridEdit.getStore();
				records.each(function(item) {
    			...
    			Ext.getCmp("report_columns_edit").setValue( Ext.util.JSON.encode(columns));
	        }
	        if(i == 3){
	    		btnSave.show();  
	            btnNext.disable();  
		...
		this.cardPanel = new Ext.Panel({
			...
			activeItem : 0,
			tbar : ['-' ,{
						id : 'jedaRpt_movePrev_edit',
						iconCls : 'button-previous',
						text : '<fmt:message key="job.stepBefore" />',
						disabled : true ,
						handler : cardHandler.createDelegate(this,[ -1 ])
					},'-',{
						id : 'jedaRpt_moveSave_edit',
						iconCls : 'button-save',
						text : '<fmt:message key="button.save" />',
						hidden : true,
						handler : this.doSave
					},'-',{
						id : 'jedaRpt_moveNext_edit',
						iconCls : 'button-next',
						text : '<fmt:message key="job.stepNext" />',
						handler : cardHandler.createDelegate(this, [ 1 ])
					}  ],
			items : [panelOneEdit,panelTwoEdit,panelThreeEdit,panelFourEdit]
		});
		
		// 设置基类属性		JedaRptEditForm.superclass.constructor.call(this, {
			labelAlign : 'right',
			labelWidth : 180 ,
			buttonAlign : 'center',
			...
			autoScroll : true,
			method : 'POST',
			url : '${ctx}/${appPath}/jedarpt/edit/${param.reportId}',
			defaults : {
				anchor : '100%',
				msgTarget : 'side'
			},
	...
	loadSuccess : function(form, action) {
		//展现路径
		var menuCode = Ext.getCmp('parent_menu_code_edit').getValue();
		Ext.Ajax.request({
			...
			success : function(resp,opts){
				...
				Ext.getCmp('parent_menu_name_edit').setValue(respText.path);
			}
		});
	},
	// 数据加载失败回调
	...
	doSave : function() {
		// 报表规则
        var rules = [];
        var recordsRule = reportRuleGridEdit.getSelectionModel().getSelections();
        for(var i = 0; i<recordsRule.length; i++) {
			rules.push(recordsRule[i].get('rule_code'));
		}
		Ext.getCmp("report_rules_edit").setValue(rules);
		// 报表角色
        var roles = [];
        var records = reportRoleGridEdit.getSelectionModel().getSelections();
        for(var i = 0; i<records.length; i++) {
			roles.push(records[i].get('role_code'));
		} 
		Ext.getCmp("report_roles_edit").setValue(roles);
		
		// 提交表单
		jedaRptEdit.getForm().submit({
			scope : jedaRptEdit,
			timeout : 1800000, //半小时			success : jedaRptEdit.saveSuccess,
			failure : jedaRptEdit.saveFailure,
			waitTitle : '<fmt:message key="message.wait" />',
...
var jedaRptEdit = new JedaRptEditForm();
//实例化新建表单,并加入到Tab页中
Ext.getCmp("JEDA_REPORT_EDIT").add(jedaRptEdit);
//刷新Tab页布局
Ext.getCmp("JEDA_REPORT_EDIT").doLayout();
