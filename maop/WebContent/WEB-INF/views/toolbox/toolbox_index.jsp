...
var Aut_tool_codes = new Array();
var Aut_appsys_codes = new Array();
var field_type_twoStore_index;
var field_type_threeStore_index;
...
tool_authorize_flagStore.load();
	toolBoxpanel = Ext.extend(
					Ext.Panel,
					{
						...
						tabIndex : 0,// 查询表单      组件Tab键顺序						csm : null,// 数据列表选择框组件						constructor : function(cfg) {// 构造方法							Ext.apply(this, cfg);
							...
							field_type_twoStore_index = new Ext.data.Store(
									{
										proxy: new Ext.data.HttpProxy({
							...
							field_type_twoStore_index.on('beforeload',function(){
								field_type_twoStore_index.baseParams.field_type_one = Ext.getCmp("field_type_one_indexID").getValue();
									},this); 
							//field_type_twoStore_index.load();
							...
							field_type_threeStore_index.on('beforeload',function(){
								field_type_threeStore_index.baseParams.field_type_one = Ext.getCmp("field_type_one_indexID").getValue();
								field_type_threeStore_index.baseParams.field_type_two = Ext.getCmp("field_type_two_indexID").getValue();
									},this); 
							...
							this.gridStore = new Ext.data.JsonStore(
									{
										...
										fields : [ 'tool_code', 'appsys_code',
												...
												'tool_received_user','tool_type','tool_content'
										],
										remoteSort : false,
										sortInfo : {
											field : 'appsys_code',
							...
							this.form = new Ext.FormPanel(
									{
										...
										items : [
												...
												{
													xtype : 'combo',
													store : field_type_twoStore_index,
													fieldLabel : '<fmt:message key="toolbox.field_type_two" />',
													id:'field_type_two_indexID',
													name : 'field_type_two',
										...
										'click' : function(node) {
											...
											this.form.getForm().reset();
												if(node.parentNode.text.indexOf("脚本")!=-1){
													Ext.getCmp("tool_typeINDEXID").setValue('1');
												}else{
													Ext.getCmp("tool_typeINDEXID").setValue('2');
												}
												
												var value =node.text.split("(")[0];
												Ext.getCmp("field_type_one_indexID").setValue(value);
												field_type_twoStore_index.load();
							...
							// 设置基类属性
							toolBoxpanel.superclass.constructor.call(this,
									{
										layout : 'border',
										border : false,
						...
						// 新建事件
						doCreate : function() {
							app.loadTab('TOOL_BOX_ALL_INFO',
											'新建工具',
											'${ctx}/${appPath}/ToolBoxController/createAllinfo');
						...
						doView : function() {
							...
							if( record.get('tool_type')==2){
								app.loadTab(
										...
										'${ctx}/${appPath}/ToolBoxController/viewDesc',
										params);
							}
						},
						// 修改事件
						doEdit : function() {
							if (this.grid.getSelectionModel().getCount() == 1) {
								...
								var params = {
									tool_code : record.get('tool_code'),
									appsys_code : record.get('appsys_code'),
									field_type_one: encodeURIComponent(record.get('field_type_one')),
									field_type_two: encodeURIComponent(record.get('field_type_two')),
									os_type : record.get('os_type'),
									server_group:record.get('server_group'),
									shell_name : record.get('shell_name')
								};
								if( record.get('tool_type')==1){
									app.loadTab(
											'TOOLBOX_EDIT',
											'脚本工具 <fmt:message key="button.edit" />',
											'${ctx}/${appPath}/ToolBoxController/edit',
											params);
								}
								if( record.get('tool_type')==2){
									app.loadTab(
											'TOOL_BOX_EDITDESC_INFO',
											'描述工具 <fmt:message key="button.edit" />',
											'${ctx}/${appPath}/ToolBoxController/editDesc',
											params);
								}
						...
						doKeyword : function() {
							if (this.grid.getSelectionModel().getCount() == 1) {
								...
								var params = {
									tool_code : record.get('tool_code'),
									appsys_code : record.get('appsys_code'),
									field_type_two :record.get('field_type_two'),
									field_type_one :record.get('field_type_one')
									
								};
								app.loadTab(
												'TOOLBOX_KEYWORDS',
												'修改匹配监控信息',
												'${ctx}/${appPath}/ToolBoxController/keywords',
												params);
							} else {
								Ext.Msg.show({
											title : '<fmt:message key="message.title" />',
						...
						doDelete : function() {
							if (this.grid.getSelectionModel().getCount() > 0) {
								...
								for ( var i = 0; i < records.length; i++) {
									...
									appsys_codes[i] = records[i].get('appsys_code');
									shell_names[i]=records[i].get('shell_name');
								}
										...
										.show({
											...
											fn : function(buttonId) {
												if (buttonId == 'ok') {
													...
													Ext.Ajax.request({
																url : '${ctx}/${appPath}/ToolBoxController/delete',
																scope : this,
						...
						//修改权限
						dotool_authorize_flag: function() {
							
							if (this.grid.getSelectionModel().getCount() > 0) {
								var records = this.grid.getSelectionModel().getSelections();
								for ( var i = 0; i < records.length; i++) {
									Aut_tool_codes.push(records[i].get('tool_code'));
									Aut_appsys_codes.push(records[i].get('appsys_code'));
						...
						doSaveAuthorize : function() {
							if(!Ext.getCmp('tool_authorize_flag_update').getValue()==""){
							Ext.Ajax.request({
								method : 'POST',
								url : '${ctx}/${appPath}/ToolBoxController/updatetool_authorize_flag',	
								scope : this,
								success : toolBox.saveSuccessAuthorize,
								failure : toolBox.saveFailureAuthorize,
								params : {
									appsyscodes:Aut_appsys_codes,
									toolcodes:Aut_tool_codes,
									tool_authorize_flag: Ext.getCmp('tool_authorize_flag_update').getValue()
								}
							});
							}else{
								Ext.Msg.show( {
						...
						// 保存成功回调
						saveSuccessAuthorize : function(form, action) {
							 ...
							 Ext.Msg.show({
									...
									fn : function() {
										Authorize_Win.close();
											app.loadTab(
													'TOOLBOX_INDEX',
													'<fmt:message key="toolbox.toolbox_edit" />',
													'${ctx}/${appPath}/ToolBoxController/index');
										
									}
							}); 
						},
						// 保存失败回调
						saveFailureAuthorize : function(form, action) {
								...
								Ext.Msg.show({
											...
											fn : function(buttonId) {
												if (buttonId == 'ok') {
													...
													Ext.Ajax.request({
																method : 'POST',
																url : '${ctx}/${appPath}/ToolBoxController/toolStatus',
																scope : this,
																success : this.toolStatusSuccess,
																failure : this.toolStatusFailure,
																params : {
																	tool_codes : tool_codes
						...
						toolStatusSuccess : function(response, options) {
							app.mask.hide();
							if (Ext.decode(response.responseText).success == false) {
								var error = Ext.decode(response.responseText).error;
								Ext.Msg
										.show({
											title : '<fmt:message key="message.title" />',
											msg : '工具交接失败'+'<fmt:message key="error.code" />:'
													+ error,
											minWidth : 200,
											buttons : Ext.MessageBox.OK,
											icon : Ext.MessageBox.ERROR
	...
	//修改权限
	var Authorize_Win = new Ext.Window({
		title:'<fmt:message key="toolbox.tool_authorize_flag" />',
		layout:'form',
		width:350,
		height:120,
		plain:true,
		...
		items : new Ext.form.FormPanel({
			...
			buttons : [{
					text :'<fmt:message key="button.save" />',
  					...
  					formBind : true,
  					scope : this,
  					handler : toolBox.doSaveAuthorize
  				},{
  					text : '<fmt:message key="button.cancel" />',
  					iconCls : 'button-cancel',
  					tabIndex : this.tabIndex++,
  					handler : toolBox.doCancelAuthorize
	...
	<sec:authorize access="hasRole('TOOL_CREATE')">
	<script type="text/javascript">
	toolBox.grid.getTopToolbar().add({
		iconCls : 'button-add',
		text : '<fmt:message key="button.add" />',
		scope : toolBox,
		handler : toolBox.doCreate
	}, '-');
</script>
</sec:authorize>

<sec:authorize access="hasRole('TOOL_EDIT')">
	<script type="text/javascript">
	toolBox.grid.getTopToolbar().add({
		iconCls : 'button-edit',
		text : '<fmt:message key="button.edit" />',
		scope : toolBox,
		handler : toolBox.doEdit
	}, '-');
</script>
</sec:authorize>

<sec:authorize access="hasRole('TOOL_DELETE')">
	<script type="text/javascript">
	toolBox.grid.getTopToolbar().add({
		iconCls : 'button-delete',
		text : '<fmt:message key="button.delete" />',
		scope : toolBox,
		handler : toolBox.doDelete
	}, '-');
</script>
</sec:authorize>

<sec:authorize access="hasRole('TOOL_AUTHORIZE_LEVEL')">
	<script type="text/javascript">
	toolBox.grid.getTopToolbar().add({
		iconCls : 'button-deploy',
		text :  '<fmt:message key="toolbox.tool_authorize_level" />',
		scope : toolBox,
		handler : toolBox.dotool_authorize_flag
	}, '-');
</script>
</sec:authorize>

<sec:authorize access="hasRole('TOOL_KEYWORD')">
	<script type="text/javascript">
	toolBox.grid.getTopToolbar().add({
		iconCls : 'button-edit',
		text :  '<fmt:message key="toolbox.tool_keyword" />',
		scope : toolBox,
		handler : toolBox.doKeyword
	}, '-');
</script>
</sec:authorize>

<sec:authorize access="hasRole('TOOL_AUTHORIZE_FLAG')">
	<script type="text/javascript">
	toolBox.grid.getTopToolbar().add({
		iconCls : 'button-deploy',
		text :'<fmt:message key="toolbox.tool_joint" />', 
		scope : toolBox,
		handler : toolBox.doToolStatus
	}, '-');
</script>
</sec:authorize>
	...
	Ext.getCmp("TOOLBOX_INDEX").add(toolBox);
	Ext.getCmp("TOOLBOX_INDEX").doLayout();
</script>
