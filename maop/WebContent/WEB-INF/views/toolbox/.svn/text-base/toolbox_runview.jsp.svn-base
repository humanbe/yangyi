...
//var task;
			...
			if (success) {
				...
				for ( var i = 0; i < data.length; i++) {
					Ext.DomHelper.append(this.body, {
						cn : '<font  style=\"font-weight: bold;font-size:15px;color:green;line-height:17px;font-family:Times New Roman\">' + data[i].info + '</font>'
					});
				}
	...
	ToolBoxViewRun = Ext.extend(Ext.Panel, {
		// 构造方法
		id:'${param.toolcode}'+'EXErunviewID',
	...
	var runView = new ToolBoxViewRun();
 	var win= Ext.getCmp('${param.toolcode}'+'EXEVIEW');
   Ext.getCmp('${param.toolcode}'+'EXEVIEW').window.get(0).add(runView);
   Ext.getCmp('${param.toolcode}'+'EXEVIEW').window.get(0).doLayout();  
	var handler = function(){
		Ext.getCmp('${param.toolcode}'+'EXErunviewID').destroy();
		Ext.getCmp('${param.toolcode}'+'EXEVIEW').window.un('close', handler);
	};
	Ext.getCmp('${param.toolcode}'+'EXEVIEW').window.on('close', handler);
</script>
