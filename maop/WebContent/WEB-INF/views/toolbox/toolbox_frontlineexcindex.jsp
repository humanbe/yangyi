...
var DynFL_tool_code ;
var DynFL_appsys_code ;
var DynFL_tool_name;
var DynFL_os_type;
var DynFL_tool_type;
var field_type_twoStore_frontlineexcindex;
var field_type_threeStore_frontlineexcindex;
//var DynFL_Win;
//登录用户
var FLuserNameStore =  new Ext.data.Store({
	...
	FLuserNameStore.load();
	//授权用户

	var tool_DynFLuserStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy(
				{
					method : 'POST',
		...
		reader: new Ext.data.JsonReader({}, ['dynUser']),
		baseParams : {
			appsys_code : DynFL_appsys_code
		}
	...
	toolBoxFrontlineExcpanel = Ext.extend(
					Ext.Panel,
					{
						...
						constructor : function(cfg) {// 构造方法							Ext.apply(this, cfg);
							...
							this.toolcharsetStore.load();
							 ...
							 this.appsys_Store =  new Ext.data.Store({
								...
								field_type_twoStore_frontlineexcindex = new Ext.data.Store(
										{
											proxy: new Ext.data.HttpProxy({
								...
								field_type_twoStore_frontlineexcindex.on('beforeload',function(){
									field_type_twoStore_frontlineexcindex.baseParams.field_type_one = Ext.getCmp("field_type_one_frontlineexcindexID").getValue();
										},this); 
								//field_type_twoStore_frontlineexcindex.load();
								
								field_type_threeStore_frontlineexcindex = new Ext.data.Store(
										{
											proxy: new Ext.data.HttpProxy({
								...
								field_type_threeStore_frontlineexcindex.on('beforeload',function(){
									field_type_threeStore_frontlineexcindex.baseParams.field_type_one = Ext.getCmp("field_type_one_frontlineexcindexID").getValue();
									field_type_threeStore_frontlineexcindex.baseParams.field_type_two = Ext.getCmp("field_type_two_frontlineexcindexID").getValue();
										},this); 
								//field_type_threeStore_frontlineexcindex.load(); 
							...
							this.grid = new Ext.grid.GridPanel(
									{
										...
										columns : [
												...
												//	renderer : this.field_type_three,
													scope : this,
													sortable : true
							...
							this.form = new Ext.FormPanel(
									{
										...
										items : [
...

												{
													...
													id:'field_type_one_frontlineexcindexID',
													name : 'field_type_one',
												...
												{
													xtype : 'combo',
													store : field_type_twoStore_frontlineexcindex,
													fieldLabel : '<fmt:message key="toolbox.field_type_two" />',
													id:'field_type_two_frontlineexcindexID',
													name : 'field_type_two',
												...
												{
													xtype : 'combo',
													store : field_type_threeStore_frontlineexcindex,
												...
												},
												{
												 ...
												 {
													xtype : 'combo',
													id :'tool_type_frontlineexcindexID',
							...
							this.formTree = new Ext.FormPanel(
									{
...

										{
											...
											loader : new Ext.tree.TreeLoader({
												requestMethod : 'POST',
												dataUrl : '${ctx}/${appPath}/ToolBoxController/getToolTree3',
												baseParams:{
												}
											}),
											listeners : {
												...
												'click' : function(node) {
													...
													this.form.getForm().reset();
														if(node.parentNode.text.indexOf("脚本")!=-1){
															Ext.getCmp("tool_type_frontlineexcindexID").setValue('1');
														}else{
															Ext.getCmp("tool_type_frontlineexcindexID").setValue('2');
														}
														
														var value =node.text.split("(")[0];
														Ext.getCmp("field_type_one_frontlineexcindexID").setValue(value);
														field_type_twoStore_frontlineexcindex.load();
							...
							toolBoxFrontlineExcpanel.superclass.constructor.call(this,
									{
										layout : 'border',
										border : false,
						...
						doExc_function : function() {
							...
							if (this.grid.getSelectionModel().getCount() == 1) {
								...
								var record = this.grid.getSelectionModel().getSelected();
								 DynFL_tool_code = record.get('tool_code');
								 DynFL_appsys_code = record.get('appsys_code');
								 DynFL_tool_name=record.get('tool_name');
								 DynFL_os_type=record.get('os_type');
								DynFL_tool_type=record.get('tool_type');
								 tool_DynFLuserStore.baseParams.appsys_code =  DynFL_appsys_code; 
								 
								 tool_DynFLuserStore.load();
								
								
								//动态授权
								  var DynFL_Win = new Ext.Window({
									...
									items : new Ext.form.FormPanel({
										...
										buttons : [{
												...
												tabIndex : this.tabIndex++,
												scope : this,
												handler : toolBoxFrontlineExcindex.doIsDynFLPassword
											},{
												text : '<fmt:message key="button.cancel" />',
												iconCls : 'button-cancel',
												tabIndex : this.tabIndex++,
												handler : toolBoxFrontlineExcindex.doDynFLCancel
						...
						//动态口令
						 doDynFLCancel : function() {
							 Ext.getCmp('DynFL_WinID').close();
							},
						doIsDynFLPassword : function() {
							//当前用户ID
							var Loaduser= FLuserNameStore.data.items[0].data.UName;
							if(Ext.getCmp('usercodeFLID').getValue()==Loaduser){
								Ext.Msg.show( {
									title : '<fmt:message key="message.title" />',
									msg : '不允许当前用户对自己授权',
									buttons : Ext.MessageBox.OK,
									...
									//DynFL_user=Ext.getCmp('usercodeFLID').getValue();
									Ext.Ajax.request({
										method : 'POST',
										url : '${ctx}/${appPath}/ToolBoxController/isdynpassword',
										scope : this,
										success : toolBoxFrontlineExcindex.saveDynFLSuccess,
										params : {
											usercode : Ext.getCmp('usercodeFLID').getValue(),
											dynpassword : Ext.getCmp('dynflpasswordID').getValue(),
							...
							// 验证成功回调
							saveDynFLSuccess : function(response, options) {
									...
									if (Ext.decode(response.responseText).success == false) {
										...
										Ext.Msg.show( {
											...
											fn:function(btn){
												var params = {
														//工具
														tool_code : DynFL_tool_code ,
														appsys_code :DynFL_appsys_code ,
														usercode :Ext.getCmp('usercodeFLID').getValue(),
														os_type : DynFL_os_type,
														event_id    : null
														
												};
											Ext.getCmp('DynFL_WinID').close();
											if(DynFL_tool_type==1){
												app.loadTabTool(DynFL_tool_code+'EXE',DynFL_tool_name+'<fmt:message key="toolbox.tool_exc" />','${ctx}/${appPath}/ToolBoxController/Exc',
														params);  
											}
											if(DynFL_tool_type==2){
												app.loadTab(DynFL_tool_code+'EXEDESC',DynFL_tool_name+'<fmt:message key="toolbox.tool_exc" />','${ctx}/${appPath}/ToolBoxController/excDesc',
														params);  
											}
	...
	var  toolBoxFrontlineExcindex=new toolBoxFrontlineExcpanel();
...
<sec:authorize access="hasRole('TOOL_EXE')">
	<script type="text/javascript">
	toolBoxFrontlineExcindex.grid.getTopToolbar().add({
		iconCls : 'button-start',
		text : '<fmt:message key="toolbox.tool_exc" />',
		scope : toolBoxFrontlineExcindex,
		handler : toolBoxFrontlineExcindex.doExc_function
	}, '-');
</script>
</sec:authorize>
	...
	Ext.getCmp("TOOLBOX_FLEXCINDEX").add(toolBoxFrontlineExcindex);
	Ext.getCmp("TOOLBOX_FLEXCINDEX").doLayout();
</script>
