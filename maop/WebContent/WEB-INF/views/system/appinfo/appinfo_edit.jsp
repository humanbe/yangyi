...
//定义新建表单
AppInfoEditForm = Ext.extend(Ext.FormPanel, {
	tabIndex : 0,// Tab键顺序
	constructor : function(cfg) {// 构造方法
		Ext.apply(this, cfg);
	
		// 设置基类属性
		AppInfoEditForm.superclass.constructor.call(this, {
			...
			frame : true,
			autoScroll : true,
			url : '${ctx}/${managePath}/appInfo/edit/'+systemcode,
			defaults : {
				anchor : '100%',
				msgTarget : 'side'
			}, 
	...
	saveSuccess : function(form, action) {
		Ext.Msg.show( {
			...
			minWidth : 200,
			fn : function() {
				app.closeTab('APP_INFO_EDIT');
				var list = Ext.getCmp("MgrAppSystemInfoIndex").get(0);
				if (list != null) {
					list.grid.getStore().reload();
				}
				app.loadTab('MgrAppSystemInfoIndex', '应用系统信息', '${ctx}/${managePath}/appInfo/index');
			}
		});
...
// 实例化新建表单,并加入到Tab页中
Ext.getCmp("APP_INFO_EDIT").add(new AppInfoEditForm());
// 刷新Tab页布局
Ext.getCmp("APP_INFO_EDIT").doLayout();
</script>