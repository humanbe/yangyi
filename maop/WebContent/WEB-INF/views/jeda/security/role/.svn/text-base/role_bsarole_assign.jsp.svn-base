...
//定义角色分配列表
BsaRoleAssignList = Ext.extend(Ext.grid.GridPanel, {
	...
	constructor : function(cfg) {// 构造方法
		...
		this.gridStore = new Ext.data.JsonStore({
			proxy : new Ext.data.HttpProxy({
				method : 'GET',
				url : '${ctx}/${frameworkPath}/role/bsarole/${param.role}',
				disableCaching : true
			}),
			autoDestroy : true,
			root : 'roles',
		...
		// 设置基类属性
		BsaRoleAssignList.superclass.constructor.call(this, {
			border : false,
			loadMask : true,
	...
	assignBsaRole : function() {
		...
		Ext.Ajax.request( {
			url : '${ctx}/${frameworkPath}/role/bsaroles',
			method : 'POST',
			scope : this,
...
app.window.get(0).add(new BsaRoleAssignList());
