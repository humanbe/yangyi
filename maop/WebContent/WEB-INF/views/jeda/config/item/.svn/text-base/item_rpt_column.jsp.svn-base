...
ItemList = Ext.extend(Ext.Panel, {
	...
	constructor : function(cfg) {// 构造方法
		...
		this.gridStore = new Ext.data.JsonStore( {
			proxy : new Ext.data.HttpProxy( {
				method : 'POST',
				url : '${ctx}/${frameworkPath}/item/rptColumn',
	...
	doSave : function() {
		...
		Ext.getCmp('column_edit_grid_create').getStore().getAt(rowIndex).set('dic_code',val);
		app.closeWindow();
	},
...
function subitem(id) {
	...
	var gridStoreSubItem = new Ext.data.JsonStore( {
		proxy : new Ext.data.HttpProxy( {
			method : 'POST',
			url : '${ctx}/${frameworkPath}/subitem/rptColumn',
			disableCaching : false
		}),
		autoDestroy : true,
		root : 'data',
