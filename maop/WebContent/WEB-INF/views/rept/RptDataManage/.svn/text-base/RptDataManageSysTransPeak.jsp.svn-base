...
//定义新建表单
RptDatasourceForm = Ext.extend(Ext.FormPanel, {
	tabIndex : 0,// Tab键顺序
	constructor : function(cfg) {// 构造方法
		aplCodeStore.load();
		Ext.apply(this, cfg);
		// 设置基类属性
		RptDatasourceForm.superclass.constructor.call(this, {
			...
			frame : true,
			autoScroll : true,
			url : '${ctx}/${managePath}/rptdatainterface/sysTransPeak',
			defaults : {
				anchor : '80%',
				msgTarget : 'side'
			},
...
// 实例化新建表单,并加入到Tab页中
Ext.getCmp("MgrRptSysTransPeak").add(new RptDatasourceForm());
// 刷新Tab页布局
Ext.getCmp("MgrRptSysTransPeak").doLayout();
