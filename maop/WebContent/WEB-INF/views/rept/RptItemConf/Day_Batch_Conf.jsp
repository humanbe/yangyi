...
function rptItemConfIndexFixedItemEdit(sheetName,dynamicCount){
	if(dynamicCount != 0){
		...
		//rptItemConfFixedItemNames = ['步骤名','开始时间','结束时间','批量状态','用时（分）'];
		//rptItemConfIndexSetVal(5,rptItemConfFixedItemNames);
		rptItemConfIndexSetStatus(0,true);
	}
...
RptItemConfEdit = Ext.extend(Ext.Panel, {
	...
	tabIndex : 0,// 查询表单组件Tab键顺序	csm : null,// 数据列表选择框组件	constructor : function(cfg) {// 构造方法		Ext.apply(this, cfg);
	    ...
	    rptItemConfform = new Ext.FormPanel({
			...
			items : [
						...
						xtype : 'textfield',
						fieldLabel : '功能描述',
						id : 'RptItemConfIndexSubSheetNameV1',
						name : 'subSheetName',
						editable : true,
						allowBlank : true,
						emptyText : '可选',
	...
	doSave : function() {
		...
		var sheetSeqParam = Ext.getCmp("RptItemConfIndexSheetSeq1").getValue().trim();
		var subSheetName = Ext.getCmp("RptItemConfIndexSubSheetNameV1").getValue().trim();
		var hostName = Ext.getCmp("RptItemConfIndexHostName1").getValue().trim();
...
if(Return_Win7!=undefined){
	Return_Win7.destroy();
}

var Return_Win7 = new Ext.Window({
	layout:'form',
	width:720,
	//height:250,
	plain:true,
...
Ext.getCmp("RptItemConfIndexSubSheetNameV1").setValue(Ext.getCmp("RptItemConfIndexSubSheetNameV").getValue());
	Ext.getCmp("RptItemConfIndexSheetSeq1").setValue(Ext.getCmp("RptItemConfIndexSheetSeq").getValue());
	Ext.getCmp("RptItemConfIndexHostName1").setValue(Ext.getCmp("RptItemConfIndexHostName").getValue());
