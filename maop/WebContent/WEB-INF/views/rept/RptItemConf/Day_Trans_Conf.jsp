...
function setItemsVisible(visibleFlag){
	Ext.getCmp("RptItemConfIndexHostName2").setValue('');
	Ext.getCmp("RptItemConfIndexHostName2").setVisible(visibleFlag);
	if(visibleFlag){
		Ext.getCmp("RptItemConfIndexHostName2").enable();
	}else{
		Ext.getCmp("RptItemConfIndexHostName2").disable();
	}
}
...
RptItemConfEdit = Ext.extend(Ext.Panel, {
	...
	tabIndex : 0,// 查询表单组件Tab键顺序	csm : null,// 数据列表选择框组件	constructor : function(cfg) {// 构造方法		Ext.apply(this, cfg);
	    ...
	    rptItemConfform = new Ext.FormPanel({
			...
			items : [
					...
					xtype : 'textfield',
					fieldLabel : '功能描述',
					id : 'RptItemConfIndexSubSheetNameV2',
					name : 'subSheetName',
					editable : true,
					allowBlank : true,
					emptyText : '可选',
	...
	doSave : function() {
		...
		var sheetSeqParam = Ext.getCmp("RptItemConfIndexSheetSeq2").getValue().trim();
		var subSheetName = Ext.getCmp("RptItemConfIndexSubSheetNameV2").getValue().trim();
		var hostName = Ext.getCmp("RptItemConfIndexHostName2").getValue().trim();
...
if(Return_Win6!=undefined){
	Return_Win6.destroy();
}

var Return_Win6 = new Ext.Window({
	layout:'form',
	width:720,
	//height:250,
	plain:true,
...
Ext.getCmp("RptItemConfIndexSubSheetNameV2").setValue(Ext.getCmp("RptItemConfIndexSubSheetNameV").getValue());
	Ext.getCmp("RptItemConfIndexSheetSeq2").setValue(Ext.getCmp("RptItemConfIndexSheetSeq").getValue());
	Ext.getCmp("RptItemConfIndexHostName2").setValue(Ext.getCmp("RptItemConfIndexHostName").getValue());
