...
function rptItemConfIndexFixedItemEdit(sheetName,dynamicCount){
	/* if(dynamicCount != 0){
		rptItemConfIndexSetStatus(dynamicCount,true);
	}else{ */
		//rptItemConfFixedItemNames = ['监控时间点','cpu使用率','内存使用率','磁盘IO使用率','五分钟交易量','前月每五分钟平均交易量','批量处理时段'];
	   //rptItemConfIndexSetVal(7,rptItemConfFixedItemNames);
		rptItemConfFixedItemNames = ['监控时间点'];
...
function setItemsVisible(visibleFlag){
	Ext.getCmp("RptItemConfIndexHostName7").setValue('');
	Ext.getCmp("RptItemConfIndexHostName7").setVisible(visibleFlag);
	if(visibleFlag){
		Ext.getCmp("RptItemConfIndexHostName7").enable();
	}else{
		Ext.getCmp("RptItemConfIndexHostName7").disable();
	}
}
...
RptItemConfEdit = Ext.extend(Ext.Panel, {
	...
	tabIndex : 0,// 查询表单组件Tab键顺序	csm : null,// 数据列表选择框组件	constructor : function(cfg) {// 构造方法		Ext.apply(this, cfg);
	    ...
	    rptItemConfform = new Ext.FormPanel({
			...
			xtype : 'textfield',
			fieldLabel : '功能描述',
			id : 'RptItemConfIndexSubSheetNameV7',
			name : 'subSheetName',
			editable : true,
			allowBlank : true,
			emptyText : '可选',
	...
	doSave : function() {
		...
		var sheetSeqParam = Ext.getCmp("RptItemConfIndexSheetSeq7").getValue().trim();
		var subSheetName = Ext.getCmp("RptItemConfIndexSubSheetNameV7").getValue().trim();
		var hostName = Ext.getCmp("RptItemConfIndexHostName7").getValue().trim();
...
if(Return_Win1!=undefined){
	Return_Win1.destroy();
}

var Return_Win1 = new Ext.Window({
	layout:'form',
	width:720,
	//height:250,
	autoScroll : true,
...
Ext.getCmp("RptItemConfIndexSubSheetNameV7").setValue(Ext.getCmp("RptItemConfIndexSubSheetNameV").getValue());
	Ext.getCmp("RptItemConfIndexSheetSeq7").setValue(Ext.getCmp("RptItemConfIndexSheetSeq").getValue());
	Ext.getCmp("RptItemConfIndexHostName7").setValue(Ext.getCmp("RptItemConfIndexHostName").getValue());
