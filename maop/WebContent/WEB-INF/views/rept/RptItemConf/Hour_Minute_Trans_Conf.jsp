...
function setItemsVisible(visibleFlag){
	Ext.getCmp("RptItemConfIndexHostName3").setValue('');
	Ext.getCmp("RptItemConfIndexHostName3").setVisible(visibleFlag);
	if(visibleFlag){
		Ext.getCmp("RptItemConfIndexHostName3").enable();
	}else{
		Ext.getCmp("RptItemConfIndexHostName3").disable();
	}
}
...
RptItemConfEdit = Ext.extend(Ext.Panel, {
	...
	tabIndex : 0,// 查询表单组件Tab键顺序	csm : null,// 数据列表选择框组件	constructor : function(cfg) {// 构造方法		Ext.apply(this, cfg);
	    ...
	    rptItemConfform = new Ext.FormPanel({
			...
			items : [
						...
						xtype : 'textfield',
						fieldLabel : '功能描述',
						id : 'RptItemConfIndexSubSheetNameV3',
						name : 'subSheetName',
						editable : true,
						allowBlank : true,
						emptyText : '可选',
	...
	doSave : function() {
		...
		var sheetSeqParam = Ext.getCmp("RptItemConfIndexSheetSeq3").getValue().trim();
		var subSheetName = Ext.getCmp("RptItemConfIndexSubSheetNameV3").getValue().trim();
		var hostName = Ext.getCmp("RptItemConfIndexHostName3").getValue().trim();
...
if(Return_Win5!=undefined){
	Return_Win5.destroy();
}

var Return_Win5 = new Ext.Window({
	layout:'form',
	width:720,
	//height:250,
	autoScroll : true,
...
Ext.getCmp("RptItemConfIndexSubSheetNameV3").setValue(Ext.getCmp("RptItemConfIndexSubSheetNameV").getValue());
	Ext.getCmp("RptItemConfIndexSheetSeq3").setValue(Ext.getCmp("RptItemConfIndexSheetSeq").getValue());
	Ext.getCmp("RptItemConfIndexHostName3").setValue(Ext.getCmp("RptItemConfIndexHostName").getValue());
