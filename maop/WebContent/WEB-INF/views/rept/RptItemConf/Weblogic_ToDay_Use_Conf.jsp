...
function setItemsVisible(visibleFlag){
	if(null==Ext.getCmp("RptItemConfIndexHostName").getValue()||Ext.getCmp("RptItemConfIndexHostName").getValue()==''){
		Ext.getCmp("RptItemConfIndexHostName9").setValue('');
	}
	Ext.getCmp("RptItemConfIndexHostName9").setVisible(visibleFlag);
	if(visibleFlag){
		Ext.getCmp("RptItemConfIndexHostName9").enable();
	}else{
		Ext.getCmp("RptItemConfIndexHostName9").disable();
	}
}
...
RptItemConfEdit = Ext.extend(Ext.Panel, {
	...
	constructor : function(cfg) {// 构造方法
	    ...
	    rptItemConfform = new Ext.FormPanel({
			...
			xtype : 'textfield',
			fieldLabel : '功能描述',
			id : 'RptItemConfIndexSubSheetNameV9',
			name : 'subSheetName',
			editable : true,
			allowBlank : true,
			emptyText : '可选',
                ...
                items : [{
					...
					listeners : {
						...
						select : function(combo, record, index){
							...
							Ext.getCmp('RptItemConfServerName' + count).setValue("");
							if(combo.getValue()=='WBLG_JDBC'){
	...
	doSave : function() {
		...
		var sheetSeqParam = Ext.getCmp("RptItemConfIndexSheetSeq9").getValue().trim();
		var subSheetName = Ext.getCmp("RptItemConfIndexSubSheetNameV9").getValue().trim();
		var hostName = Ext.getCmp("RptItemConfIndexHostName9").getValue().trim();
...
Ext.getCmp("RptItemConfIndexSubSheetNameV9").setValue(Ext.getCmp("RptItemConfIndexSubSheetNameV").getValue());
Ext.getCmp("RptItemConfIndexSheetSeq9").setValue(Ext.getCmp("RptItemConfIndexSheetSeq").getValue());
Ext.getCmp("RptItemConfIndexHostName9").setValue(Ext.getCmp("RptItemConfIndexHostName").getValue());
...
if('${param.itemNameOnes}'==''){
	...
	RptItemConfWeblogicDataStore.baseParams={
			...
			sheetName : encodeURIComponent(decodeURIComponent('${param.sheetNameWeblogic}'))
	};
