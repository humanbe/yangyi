...
function setItemsVisible(visibleFlag){
	Ext.getCmp("RptItemConfIndexHostName5").setValue('');
	Ext.getCmp("RptItemConfIndexHostName5").setVisible(visibleFlag);
	if(visibleFlag){
		Ext.getCmp("RptItemConfIndexHostName5").enable();
	}else{
		Ext.getCmp("RptItemConfIndexHostName5").disable();
	}
}
...
RptItemConfEdit = Ext.extend(Ext.Panel, {
	...
	tabIndex : 0,// 查询表单组件Tab键顺序	csm : null,// 数据列表选择框组件	constructor : function(cfg) {// 构造方法		Ext.apply(this, cfg);
	    ...
	    rptItemConfform = new Ext.FormPanel({
			...
			xtype : 'textfield',
			fieldLabel : '功能描述',
			id : 'RptItemConfIndexSubSheetNameV5',
			name : 'subSheetName',
			editable : true,
			allowBlank : true,
			emptyText : '可选',
	...
	doSave : function() {
		...
		var sheetSeqParam = Ext.getCmp("RptItemConfIndexSheetSeq5").getValue().trim();
		var subSheetName = Ext.getCmp("RptItemConfIndexSubSheetNameV5").getValue().trim();
		var hostName = Ext.getCmp("RptItemConfIndexHostName5").getValue().trim();
...
if(Return_Win3!=undefined){
	Return_Win3.destroy();
}

var Return_Win3 = new Ext.Window({
	layout:'form',
	width:720,
//	height:250,
   autoScroll : true,
...
Ext.getCmp("RptItemConfIndexSubSheetNameV5").setValue(Ext.getCmp("RptItemConfIndexSubSheetNameV").getValue());
	Ext.getCmp("RptItemConfIndexSheetSeq5").setValue(Ext.getCmp("RptItemConfIndexSheetSeq").getValue());
	Ext.getCmp("RptItemConfIndexHostName5").setValue(Ext.getCmp("RptItemConfIndexHostName").getValue());
