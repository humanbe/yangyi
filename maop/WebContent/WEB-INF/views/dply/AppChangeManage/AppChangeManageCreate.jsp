...
AppChangeManageCreateForm = Ext.extend(Ext.FormPanel, {
	...
	constructor : function(cfg) {// 构造方法
		...
		this.arrangeStore = new Ext.data.JsonStore(
              ...
              arrangeCreateGrid = new Ext.grid.EditorGridPanel(
				{
					region : 'center',
					border : false,
					loadMask : true,
			...
			items : [
				{
					...
					listeners : {
						...
						select : function(combo, record, index){
							this.getForm().findField('aplCode').setValue(combo.value);
							this.arrangeStore.baseParams.appsysCode=combo.value;
							if(this.getForm().findField('changeDate').getValue()!=''){
						...
						select : function(obj){
							...
							Ext.Ajax.request({
								...
								disableCaching:true,
								params : {
									appsysCode : Ext.getCmp('appsysCode_Create').getValue()
								},
								success: function(response) {
									var data = Ext.decode(response.responseText).data;
									if(1 != data){
										Ext.getCmp('tabCreate').hideTabStripItem('sortCreate');
									}else{
										Ext.getCmp('tabCreate').unhideTabStripItem('sortCreate');
										}
								}, 
				...
				},
				{
					...
					listeners : {
						scope : this,
						select : function(o){
							this.arrangeStore.baseParams.changeDate=o.value;
							if(this.getForm().findField('aplCode').getValue()!=''){
								this.arrangeStore.load();
							}
						}
					}
		            	...
		            	height:440,
						 ...
						 listeners : {
								...
								var planEndTime=	this.getForm().findField('planEndTime').getValue().replace(':',''); //排期结束时间
					                ...
					                setTimeout(function(){
					                	arrangeCreateGrid.getSelectionModel().selectRecords(records, false);
					                }, 500);
								} 
							}
		            }
	...
	doSave : function() {
		...
		var f='';
		var jsonRequests = [];
		var sm =arrangeCreateGrid.getSelectionModel().getSelections();
