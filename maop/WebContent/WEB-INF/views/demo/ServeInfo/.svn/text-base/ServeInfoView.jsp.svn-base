...
ServeInfoViewForm = Ext.extend(Ext.FormPanel, {
	tabIndex : 0,// Tab键顺序
	constructor : function(cfg) {// 构造方法
		Ext.apply(this, cfg);
		
		// 设置基类属性
		ServeInfoViewForm.superclass.constructor.call(this, {
			title : '<fmt:message key="title.form" />',
			labelAlign : 'right',
			buttonAlign : 'center',
			frame : true,
			autoScroll : true,
			url : '${ctx}/${appPath}/serveinfo/edit/${param.id}',
			defaults : {
				anchor : '80%',
				msgTarget : 'side'
			},
		...
		// 加载表单数据
		this.load( {
			url : '${ctx}/${appPath}/serveinfo/view/${param.id}',
			method : 'GET',
...
// 实例化新建表单,并加入到Tab页中
Ext.getCmp("view_ServeInfo").add(new TranInfoViewForm());
// 刷新Tab页布局
Ext.getCmp("view_ServeInfo").doLayout();
</script>