...
//定义列表
RelServiceInfoList = Ext.extend(Ext.Panel, {
	...
	constructor : function(cfg) {// 构造方法
		...
		this.gridStore = new Ext.data.JsonStore( {
			proxy : new Ext.data.HttpProxy( {
				method : 'POST',
				url : '${ctx}/${appPath}/relserviceinfo/index',
...
//		this.gridStore.load();
		...
		this.grid = new Ext.grid.GridPanel( {
			id : 'RelServiceInfoListGridPanel',
			region : 'center',
			border : false,
			loadMask : true,
		...
		this.form = new Ext.FormPanel( {
			id : 'RelServiceInfoFindFormPanel',
			region : 'east',
			...
			items : [ 
				...
				{
				    xtype : 'textfield',
					fieldLabel : '<fmt:message key="property.outserivcesid" />',
					name : 'outserivcesid',
					tabIndex : this.tabIndex++,
					allowBlank : false
				},
		...
		// 设置基类属性
		RelServiceInfoList.superclass.constructor.call(this, {
			layout : 'border',
			border : false,
	...
	doView : function() {
		...
		app.loadTab('view_RelServiceInfo', '<fmt:message key="button.view" /><fmt:message key="RelServiceInfo" />', '${ctx}/${appPath}/relserviceinfo/view', params);
	},
	// 新建事件
	doCreate : function() {
		app.loadTab('add_RelServiceInfo', '<fmt:message key="button.create" /><fmt:message key="RelServiceInfo" />', '${ctx}/${appPath}/relserviceinfo/create');
	},
	// 编辑事件
    doEdit : function() {
		if (this.grid.getSelectionModel().getCount() == 1) {
			...
			app.loadTab('edit_RelserviceInfo', '<fmt:message key="button.edit" /><fmt:message key="RelServiceInfo" />', '${ctx}/${appPath}/relserviceinfo/edit', params);
		} else {
			Ext.Msg.show( {
			...
			Ext.Msg.show( {
				...
				fn : function(buttonId) {
					if (buttonId == 'ok') {
						app.mask.show();
						Ext.Ajax.request( {
							url : '${ctx}/${appPath}/relserviceinfo/delete',
							scope : this,
...
Ext.getCmp("RelServiceInfoIndex").add(new RelServiceInfoList());
Ext.getCmp("RelServiceInfoIndex").doLayout();
</script>
