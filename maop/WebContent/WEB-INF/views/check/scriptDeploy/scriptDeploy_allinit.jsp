...
<script type="text/javascript">
	...
	var initGridPanel = null;
	CheckInitFrom = Ext.extend(Ext.FormPanel,
					{
						...
						tabIndex : 0,// Tab键顺序						constructor : function(cfg) {
							...
							//必选参数数据源
						 	this.checkInitPanelStore = new Ext.data.JsonStore({
								...
								{
									method : 'POST',
									url : '${ctx}/${managePath}/scriptDeploy/queryAllInit',
									disableCaching : true
								}),
								autoDestroy : true,
								root : 'data',
										...
										items : [ {
											...
											items : [
													...
													},
													{
														...
														xtype : 'textfield',
														id : 'allInitScriptNameId',
														hiddenName : 'allInitScriptName',
														name : 'allInitScriptName',
														tabIndex : this.tabIndex++,
														hidden : true
													},{
														xtype : 'textfield',
														id : 'initServerIpId',
														hiddenName : 'initServerIp',
														name : 'initServerIp',
														tabIndex : this.tabIndex++,
														hidden : true
													},{
							...
							this.serverIpPanelStore = new Ext.data.JsonStore({
								...
								{
									method : 'POST',
									url : '${ctx}/${managePath}/scriptDeploy/queryServerIp',
									disableCaching : true
								}),
								autoDestroy : true,
								root : 'data',
							...
							// 设置基类属性							CheckInitFrom.superclass.constructor.call(this,{
												labelAlign : 'right',
												buttonAlign : 'center',
												frame : true,
												autoScroll : true,
												url : '${ctx}/${managePath}/scriptDeploy/exec',
												defaults : {
													anchor : '100%',
													msgTarget : 'side'
												},
						...
						doSave : function() {
							...
							Ext.getCmp('initId').setValue(Ext.util.JSON.encode(paramJson));
							...
							Ext.getCmp('initServerIpId').setValue(Ext.util.JSON.encode(jsonServer));
							var allInitScriptName='${param.allinitNshScriptName}';
							Ext.getCmp('allInitScriptNameId').setValue(allInitScriptName);
							if(flag!='n'){
								for ( var j = 0; j < records.length; j++) {
									if(records.length==1 && records[j].get('scriptType')=='执行'){
									Ext.Msg.show({
										title : '<fmt:message key="message.title" />',
										msg : '请先下发脚本',
										buttons : Ext.MessageBox.OK,
							...
							checkInitFrom.getForm().submit(
									{
										scope : checkInitFrom,
										timeout : 1000000, 
										success : checkInitFrom.saveSuccess,
										failure : checkInitFrom.saveFailure,
						...
						doFind : function() {
							Ext.apply(this.serverIpPanelStore.baseParams, {
								serverIp : Ext.getCmp('initInputip').getValue()
							});
							serverIpPanel.getStore().load();
						},
						// 取消操作
						doCancel : function() {
							app.closeTab('add_allInit');
						},
						// 保存成功回调
						saveSuccess : function(form, action) {
							Ext.Msg.show({
										...
										minWidth : 200,
										fn : function() {
											app.closeTab('add_scriptDeploy');
											var list = Ext.getCmp(
													"SCRIPT_ALLINIT").get(0);
											if (list != null) {
												list.grid.getStore().reload();
											}
											app.loadTab('SCRIPT_ALLINIT','<fmt:message key="button.view" /><fmt:message key="function" />',
															'${ctx}/${managePath}/scriptDeploy/allindex');
										}
									});
						},
						// 保存失败回调
	...
	var checkInitFrom = new CheckInitFrom();
</script>
<script>
	// 实例化新建表单,并加入到Tab页中
	Ext.getCmp("add_allInit").add(checkInitFrom);
	// 刷新Tab页布局
	Ext.getCmp("add_allInit").doLayout();
</script>