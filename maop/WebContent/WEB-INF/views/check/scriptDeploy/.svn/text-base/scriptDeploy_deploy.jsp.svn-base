	...
	ScriptUploadForm = Ext.extend(Ext.FormPanel,
					{
						...
						tabIndex : 0,// Tab键顺序						constructor : function(cfg) {
							...
							//必选参数数据源
						 	this.checkInitPanelStore = new Ext.data.JsonStore({
								...
								{
									method : 'POST',
									url : '${ctx}/${managePath}/scriptDeploy/queryInit',
									disableCaching : true
								}),
								autoDestroy : true,
								root : 'data',
							...
							// 设置基类属性							ScriptUploadForm.superclass.constructor.call(this,{
												...
												frame : true,
												autoScroll : true,
												url : '${ctx}/${managePath}/scriptDeploy/create',
												defaults : {
													anchor : '100%',
													msgTarget : 'side'
												},
						...
						doSave : function() {
							...
							Ext.getCmp('scriptDeployValueId').setValue(Ext.util.JSON.encode(paramJson));
							...
							Ext.getCmp('deployValueId').setValue(Ext.util.JSON.encode(jsonServer));
							var binScriptName='${param.binScriptName}';
							Ext.getCmp('binScriptNameId').setValue(binScriptName);
							var osTypes='${param.osType}';
							Ext.getCmp('osTypesId').setValue(osTypes);
							var fieldTypes='${param.fieldType}';
							Ext.getCmp('fieldTypesId').setValue(fieldTypes);
							...
							Ext.Msg.show({
										...
										fn : function() {
											...
											app.loadTab('SCRIPT_DEPLOY','<fmt:message key="button.view" /><fmt:message key="function" />',
															'${ctx}/${managePath}/scriptDeploy/index');
										}
									});
						},
						// 保存失败回调
...
<script>
	// 实例化新建表单,并加入到Tab页中
	Ext.getCmp("add_scriptDeploy").add(scriptUploadForm);
	// 刷新Tab页布局
	Ext.getCmp("add_scriptDeploy").doLayout();
</script>