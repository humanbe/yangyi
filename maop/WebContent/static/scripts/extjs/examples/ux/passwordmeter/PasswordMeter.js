 ...
 * http://digitalspaghetti.me.uk/index.php?q=jquery-pstrength and Steve Moitozo
 ...
 * @constructor Creates new Ext.ux.PasswordMeter
 */
Ext.ux.PasswordMeter = function(config) {
	...
	if (config.calcStrength) {
		this.calcStrength = config.calcStrength;
	}
...
Ext.extend(Ext.ux.PasswordMeter, Ext.form.TextField, {
	/**
	 * @cfg {String} inputType The type attribute for input fields -- e.g. text,
...
// private
	...
	updateMeter : function(e) {
		...
		scoreWidth = (maxWidth / 100) * nScore;
	...
	if (p.match(new RegExp(".*\\d.*\\d.*\\d"))) { // [verified] at least three
													// numbers
		intScore += 5;
	}

	// SPECIAL CHAR
	if (p.match(new RegExp("[!,@,#,$,%,^,&,*,?,_,~]"))) { // [verified] at
															// least one special
															// character
		intScore += 5;
	}
	...
	// COMBOS
	if (p.match(new RegExp("(?=.*[a-z])(?=.*[A-Z])"))) { // [verified] both
															// upper and lower
															// case
		intScore += 2;
	}
	if (p.match(new RegExp("(?=.*\\d)(?=.*[a-z])(?=.*[A-Z])"))) { // [verified]
																	// both
																	// letters
																	// and
																	// numbers
